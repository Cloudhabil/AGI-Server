...
GENESIS_CONSTANT = 0.0022      # Degradation threshold
BETA_SECURITY = 0.236          # Optimal DoD window (23.6% from edges)
PHI = 1.618033988749895        # Golden ratio for charge rates
...
class BrahimBatteryCalculator:
    ...
    def find_optimal_battery(
        ...
        for code, chem in self.chemistries.items():
            ...
            cycles_lifetime = chem.cycle_life * chem.dod_max
...
def visualize_chemistry_deep_dive(code: str) -> str:
    ...
    lines.append(f"    Integrity:         {'█' * int(integrity * 10)}{'░' * (10 - int(integrity * 10))} ({integrity:.2f})")
    lines.append(f"    Grid resonance:    {'█' * int(resonance * 10)}{'░' * (10 - int(resonance * 10))} ({resonance:.2f})")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Brahim Battery Materials Optimizer",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python battery_optimizer.py                          # Find optimal batteries
  python battery_optimizer.py --materials              # Material integrity
  python battery_optimizer.py --chemistry lfp          # Deep dive LFP
  python battery_optimizer.py --application utility    # Utility-scale options
  python battery_optimizer.py --duration seasonal      # Long-duration storage
        """,
    )
    ...
    parser.add_argument(
        "--materials", "-m",
        action="store_true",
        help="Show material integrity analysis",
    )
    parser.add_argument(
        "--chemistry", "-c",
        help="Deep dive into specific chemistry (e.g., lfp, sodium_ion)",
    )
    parser.add_argument(
        "--application", "-a",
        choices=["residential", "commercial", "utility", "grid"],
        default="utility",
        help="Application scale",
    )
    parser.add_argument(
        "--duration", "-d",
        choices=["short", "medium", "long", "seasonal"],
        default="medium",
        help="Storage duration requirement",
    )
    parser.add_argument(
        "--budget",
        type=float,
        default=500,
        help="Maximum $/kWh budget",
    )
    parser.add_argument(
        "--integrity-first",
        action="store_true",
        help="Prioritize supply chain integrity",
    )
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    if args.formula:
        print("""
╔═══════════════════════════════════════════════════════════════════════════╗
║           BRAHIM BATTERY OPTIMIZATION FORMULAS                            ║
╠═══════════════════════════════════════════════════════════════════════════╣
║                                                                           ║
║  1. BATTERY STRESS (Degradation):                                         ║
║     B(t) = Σ(1/(max_cycles - used)²) × exp(-λ × DoD)                      ║
║                                                                           ║
║  2. MATERIAL INTEGRITY (Anti-Oppression):                                 ║
║     I = (1 - supply_risk) × (1 - environmental) × recyclability           ║
║                                                                           ║
║  3. GRID RESONANCE (Response Match):                                      ║
║     R = exp(-λ × |response_time - optimal|)                               ║
║                                                                           ║
║  4. OPTIMAL BATTERY SCORE:                                                ║
║     M = (energy × power × cycles × efficiency) / (cost × B) × R × I       ║
║                                                                           ║
╠═══════════════════════════════════════════════════════════════════════════╣
║  BRAHIM CONSTANTS:                                                        ║
║    GENESIS_CONSTANT = 0.0022  (degradation threshold)                     ║
║    BETA_SECURITY = 0.236      (optimal DoD margin: 23.6% each end)        ║
║    PHI = 1.618...             (charge rate optimization)                  ║
║    DOD_OPTIMAL = 52.8%        (usable capacity in safe window)            ║
║                                                                           ║
╠═══════════════════════════════════════════════════════════════════════════╣
║  INTEGRITY PRINCIPLE:                                                     ║
║    Materials controlled by single sources score LOWER.                    ║
║    Diverse supply chains = protection against oppression/manipulation.   ║
║    Recyclability = circular economy = sustainable integrity.              ║
║                                                                           ║
╚═══════════════════════════════════════════════════════════════════════════╝
        """)
        return
    ...
    # Materials analysis
    if args.materials:
        print(visualize_materials())
        return

    # Chemistry deep dive
    if args.chemistry:
        print(visualize_chemistry_deep_dive(args.chemistry))
        return

    # Find optimal batteries
    app_scale = ApplicationScale(args.application)
    duration = DurationClass(args.duration)

    results = calc.find_optimal_battery(
        application=app_scale,
        duration=duration,
        budget_per_kwh=args.budget,
        prioritize_integrity=args.integrity_first,
    )
    ...
    if args.json:
        output = [
            {
                "rank": r.rank,
                "chemistry": r.chemistry.name,
                "code": r.chemistry.code,
                "brahim_score": round(r.brahim_score, 4),
                "integrity_score": round(r.integrity_score, 4),
                "cost_per_kwh": r.chemistry.cost_per_kwh,
                "efficiency": r.chemistry.round_trip_efficiency,
                "cycle_life": r.chemistry.cycle_life,
        ...
        if results:
            print(f"  TOP RECOMMENDATION: {results[0].chemistry.name}")
            print(f"  {results[0].recommendation}")
            print()

            # Show why
            top = results[0]
            print(f"  Why {top.chemistry.code}?")
            print(f"    - Integrity score: {top.integrity_score:.2f} (supply chain security)")
            print(f"    - Grid resonance: {top.grid_resonance:.2f} (matches renewable variability)")
            print(f"    - Cost: ${top.chemistry.cost_per_kwh}/kWh")
            print(f"    - Lifetime CO2 savings: {top.co2_savings_kg_per_kwh * top.chemistry.cycle_life / 1000:.0f} tons per MWh installed")
            print()
