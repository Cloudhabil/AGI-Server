#!/usr/bin/env python3
"""
US Grid Stress Formula - Standalone Module
===========================================
...
USAGE:
    python us_grid_stress.py                     # Run demo (CAISO)
    python us_grid_stress.py --iso ERCOT         # Texas grid
    python us_grid_stress.py --compare           # All ISOs comparison
    python us_grid_stress.py --duck-curve        # California duck curve
    python us_grid_stress.py --extreme-weather   # Stress test scenarios
"""
...
EXTREME_WEATHER_SCENARIOS: Dict[str, ExtremeWeatherEvent] = {
    ...
    "heat_dome": ExtremeWeatherEvent(
        ...
        historical_example="June 2021 Pacific Northwest - 116°F in Portland",
    ),
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Brahim Grid Stress Calculator for US Grids",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python us_grid_stress.py                          # CAISO demo
  python us_grid_stress.py --iso ERCOT              # Texas grid
  python us_grid_stress.py --compare                # All ISOs
  python us_grid_stress.py --duck-curve             # California duck
  python us_grid_stress.py --iso ERCOT --weather polar_vortex
        """,
    )
    ...
    parser.add_argument(
        "--iso", "-i",
        default="CAISO",
        choices=[iso.value.upper() for iso in US_ISO_CONFIGS.keys()],
        help="ISO/RTO region (default: CAISO)",
    )
    parser.add_argument(
        "--compare",
        action="store_true",
        help="Compare all US ISOs",
    )
    parser.add_argument(
        "--duck-curve",
        action="store_true",
        help="Show California duck curve",
    )
    parser.add_argument(
        "--weather", "-w",
        choices=list(EXTREME_WEATHER_SCENARIOS.keys()),
        help="Simulate extreme weather event",
    )
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    parser.add_argument(
        "--co2-savings",
        action="store_true",
        help="Calculate annual CO2 savings potential",
    )
    ...
    if args.formula:
        print("""
╔═══════════════════════════════════════════════════════════════════════╗
║              BRAHIM GRID STRESS FORMULA (US GRIDS)                    ║
╠═══════════════════════════════════════════════════════════════════════╣
║                                                                       ║
║   G(t) = Σ(1/(capacity - demand)²) × exp(-λ × renewable_fraction)     ║
║                                                                       ║
╠═══════════════════════════════════════════════════════════════════════╣
║  STRESS LEVELS:                                                       ║
║    G < 0.0001  : EXCELLENT - Shift all loads (negative prices!)       ║
║    G < 0.001   : GOOD - Great for EV charging & data centers          ║
║    G < 0.01    : MODERATE - Normal operations                         ║
║    G < 0.1     : HIGH - Conservation advisory                         ║
║    G < 1.0     : CRITICAL - Flex Alert issued                         ║
║    G >= 1.0    : EMERGENCY - Rolling blackouts imminent               ║
║                                                                       ║
╠═══════════════════════════════════════════════════════════════════════╣
║  US-SPECIFIC FEATURES:                                                ║
║    • Duck Curve (CAISO): Solar belly 10-15, neck ramp 17-20           ║
║    • Wind Corridor (ERCOT/SPP/MISO): Best at night                    ║
║    • Polar Vortex: +45% demand, -60% wind (ERCOT Feb 2021)            ║
║    • Heat Dome: +35% demand, AC strain (Western US)                   ║
║                                                                       ║
╚═══════════════════════════════════════════════════════════════════════╝
        """)
        return
    ...
    # Duck curve
    if args.duck_curve:
        print(visualize_duck_curve())
        return

    # US comparison
    if args.compare:
        print(visualize_us_comparison())
        return

    # Get ISO
    iso = ISORegion(args.iso.lower())
    calc = USGridStressCalculator(iso)

    # CO2 savings
    if args.co2_savings:
        # US-scale numbers
        savings = calc.calculate_annual_co2_savings(
            ev_count=5_000_000,        # 5M EVs
            data_center_mw=5000,       # 5 GW data centers
            industrial_mw=10000,       # 10 GW industrial
            residential_homes=50_000_000,  # 50M homes
        )

        print(f"\n  Annual CO2 Savings Potential - {calc.config.name}")
        print("  " + "=" * 50)
        for key, value in savings.items():
            print(f"  {key:20s}: {value:>12,.0f} tons CO2/year")
        print()
        return

    # Extreme weather
    if args.weather:
        print(visualize_extreme_weather(iso, args.weather))
        return

    # Optimal windows
    if args.optimal:
        windows = calc.find_optimal_windows(duration_hours=args.optimal)
        if args.json:
            print(json.dumps([
                {
                    "start": w[0].isoformat(),
            ...
            print(f"\n  Optimal {args.optimal}h Windows - {calc.config.name}:")
            print("  " + "=" * 50)
            for start, end, stress, price in windows[:5]:
                print(f"  {start.strftime('%H:%M')} - {end.strftime('%H:%M')}: "
                      f"stress={stress:.6f}, ${price:.1f}/MWh")
            print()
        return

    # Standard 24h
    if args.json:
        simulation = calc.simulate_24h(weather_event=args.weather)
        output = {
            "iso": calc.config.name,
            "code": calc.config.code,
            "interconnection": calc.config.interconnection.value,
            "timestamp": datetime.now().isoformat(),
            "weather_event": args.weather,
            "hours": [{"time": t.isoformat(), **r.to_dict()} for t, r in simulation],
        }
        print(json.dumps(output, indent=2))
    else:
        print(visualize_24h(calc, args.weather))

        windows = calc.find_optimal_windows(duration_hours=2.0)
        if windows:
            print(f"  Best 2-hour windows:")
            for start, end, stress, price in windows[:3]:
                print(f"    {start.strftime('%H:%M')} - {end.strftime('%H:%M')} (${price:.0f}/MWh)")
            print()
