...
BATCH_SIZE_FIBONACCI = 34    # From Brahim sequence
EPOCHS_RESONANCE = 89        # From Brahim sequence
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Brahim Material Engine Agent",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python material_engine_agent.py --analyze LFP
  python material_engine_agent.py --discover --target-energy 300
  python material_engine_agent.py --integrity lithium,cobalt,graphite
  python material_engine_agent.py --status
        """,
    )

    parser.add_argument("--analyze", help="Analyze a chemistry (e.g., lfp, nmc)")
    parser.add_argument("--discover", action="store_true", help="Discover new materials")
    parser.add_argument("--target-energy", type=int, default=200, help="Target energy density")
    parser.add_argument("--target-cycles", type=int, default=5000, help="Target cycle life")
    parser.add_argument("--integrity", help="Check integrity of materials (comma-separated)")
    ...
    if args.integrity:
        materials = [m.strip() for m in args.integrity.split(",")]
        result = agent.models["integrity_scorer"].predict({"materials": materials})
        if args.json:
    ...
    if args.analyze:
        chemistry = args.analyze.lower()
        result = agent.process_next_task() or {}

        # Run full analysis
        perception = agent.perceive({"chemistry": chemistry, "type": "chemistry_analysis"})
        reasoning = agent.reason(perception)
        actions = agent.act(reasoning)

        if args.json:
            print(json.dumps(actions, indent=2, default=str))
        else:
            print(f"\n  Chemistry Analysis: {chemistry.upper()}")
            print(f"  {'=' * 50}")

            if "safety" in reasoning["predictions"]:
                safety = reasoning["predictions"]["safety"]
                print(f"  Safety Level: {safety['risk_level']}")
                print(f"  Risk Score: {safety['risk_score']:.4f}")

            print(f"\n  Recommendations:")
            for rec in reasoning.get("recommendations", []):
                print(f"    - {rec}")

            print(f"\n  Compliance: {', '.join(reasoning.get('compliance', []))}")
            print()
        return

    if args.discover:
        targets = {
            "energy_density_wh_kg": args.target_energy,
            "cycle_life": args.target_cycles,
        }
        result = agent.discover_material(targets)

        if args.json:
            print(json.dumps(result, indent=2, default=str))
        else:
            print(f"\n  Material Discovery Results")
            print(f"  {'=' * 50}")
            print(f"  Targets: Energy={args.target_energy} Wh/kg, Cycles={args.target_cycles}")
            print()
            ...
            for i, candidate in enumerate(result["candidates"][:3]):
                ...
                print(f"    Integrity: {candidate['integrity']['integrity_score']:.4f}")
                print()
        return
