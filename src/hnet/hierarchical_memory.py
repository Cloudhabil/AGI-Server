...
class HierarchicalMemory:
    ...
    def search(
        ...
        top_k: int = 5,
        *,
        conversation_match: str = "exact",
    ) -> List[str]:
        """
        Return up to ``top_k`` matching chunks for ``query``.

        Args:
            conversation_id: Identifier or prefix to filter stored chunks. When None,
                no filtering is applied.
            query: Text to embed and search for.
            top_k: Maximum number of results to return.
            conversation_match: Matching strategy - "exact" or "prefix".
        """
        ...
        for idx in idxs[0]:
            ...
            # Allow prefix-based filtering so grouped IDs (e.g., "ledger:foo") can
            # be retrieved via a shared prefix (e.g., "ledger").
            if conversation_id is not None:
                if conversation_match == "prefix":
                    if not (isinstance(meta_cid, str) and meta_cid.startswith(conversation_id)):
                        continue
                else:
                    if meta_cid != conversation_id:
                        continue
