...
def main():
    ...
    result = registry.execute_skill('research/verification-harness', payload, context=None)
    emit('EXECUTE', {'skill_id': 'research/verification-harness', 'success': result.success})
    emit('VERIFY', {'success': bool(result.success)})
    emit('COMMIT', {'record': {'skill_id': 'research/verification-harness', 'success': result.success}})
    if result.success:
