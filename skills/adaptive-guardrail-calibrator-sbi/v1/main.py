...
def main():
    ...
    result = registry.execute_skill('automation/adaptive-guardrail-calibrator', payload, context=None)
    emit('EXECUTE', {'skill_id': 'automation/adaptive-guardrail-calibrator', 'success': result.success})
    emit('VERIFY', {'success': bool(result.success)})
    emit('COMMIT', {'record': {'skill_id': 'automation/adaptive-guardrail-calibrator', 'success': result.success}})
    if result.success:
