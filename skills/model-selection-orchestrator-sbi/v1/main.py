...
def main():
    ...
    result = registry.execute_skill('tuning/model-selection-orchestrator', payload, context=None)
    emit('EXECUTE', {'skill_id': 'tuning/model-selection-orchestrator', 'success': result.success})
    emit('VERIFY', {'success': bool(result.success)})
    emit('COMMIT', {'record': {'skill_id': 'tuning/model-selection-orchestrator', 'success': result.success}})
    if result.success:
