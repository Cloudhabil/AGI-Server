...
def main():
    ...
    result = registry.execute_skill('interface/vue-threejs-skilltree', payload, context=None)
    emit('EXECUTE', {'skill_id': 'interface/vue-threejs-skilltree', 'success': result.success})
    emit('VERIFY', {'success': bool(result.success)})
    emit('COMMIT', {'record': {'skill_id': 'interface/vue-threejs-skilltree', 'success': result.success}})
    if result.success:
