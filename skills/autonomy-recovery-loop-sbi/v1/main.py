...
def main():
    ...
    result = registry.execute_skill('automation/autonomy-recovery-loop', payload, context=None)
    emit('EXECUTE', {'skill_id': 'automation/autonomy-recovery-loop', 'success': result.success})
    emit('VERIFY', {'success': bool(result.success)})
    emit('COMMIT', {'record': {'skill_id': 'automation/autonomy-recovery-loop', 'success': result.success}})
    if result.success:
