...
def main():
    ...
    result = registry.execute_skill('automation/dynamic-budget-orchestrator', payload, context=None)
    emit('EXECUTE', {'skill_id': 'automation/dynamic-budget-orchestrator', 'success': result.success})
    emit('VERIFY', {'success': bool(result.success)})
    emit('COMMIT', {'record': {'skill_id': 'automation/dynamic-budget-orchestrator', 'success': result.success}})
    if result.success:
