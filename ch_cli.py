...
def _run_to_model(key, user_prompt, tdir, meta, system):
    ...
    print(
        f"➡️  Modelo: {model_cfg.get('name', key)} | Motivo: {meta.get('tipo', 'n/a')} | Tam: {meta.get('tam', '?')}"
    )
...
def cmd_run(args):
    ...
    tdir = ensure_task_dir(slug)
    context = (
        (tdir / "inputs" / "context.md").read_text(encoding="utf-8")
        if (tdir / "inputs" / "context.md").exists()
        else ""
    )
    task = (
        (tdir / "inputs" / "tarea.md").read_text(encoding="utf-8")
        if (tdir / "inputs" / "tarea.md").exists()
        else args.task or ""
    )
    constraints = (
        (tdir / "inputs" / "restricciones.md").read_text(encoding="utf-8")
        if (tdir / "inputs" / "restricciones.md").exists()
        else args.constraints or ""
    )
...
def cmd_doctor(args):
    ...
    for key in ["generator_primary", "assistant_qc"]:
        ...
        if _is_alive:
            ...
            print(
                f" - {cfg.get('name', key)}: {'OK' if ok else 'DOWN'} @ {cfg.get('base_url', '')}"
            )
        else:
            print(
                f" - {cfg.get('name', key)}: base_url={cfg.get('base_url', '')} (no _is_alive helper)"
            )


def main():
    # --- Dateiverknüpfung: Prüfen ob eine Datei übergeben wurde ---
    if len(sys.argv) > 1 and not sys.argv[1].startswith("-"):
        first_arg = sys.argv[1]

        # Prüfen ob das Argument eine existierende Datei ist
        if os.path.exists(first_arg):
            file_path = Path(first_arg).resolve()

            # Log in Datei schreiben (da --noconsole keine Konsole zeigt)
            log_file = ROOT / "file_association_log.txt"
            try:
                with open(log_file, "w", encoding="utf-8") as f:
                    f.write(f"CLI AI wurde mit Datei gestartet:\n")
                    f.write(f"Dateipfad: {file_path}\n")
                    f.write(f"Dateiname: {file_path.name}\n")
                    f.write(f"Dateierweiterung: {file_path.suffix}\n")
                    f.write(f"\nHier können Sie die Datei verarbeiten...\n")
            except Exception as e:
                pass  # Fehler beim Logging ignorieren

            # Optional: Windows MessageBox zeigen (nur wenn tkinter verfügbar ist)
            try:
                ...
                messagebox.showinfo(
                    "CLI AI - Datei geöffnet",
                    f"CLI AI wurde mit folgender Datei gestartet:\n\n{file_path}\n\n"
                    f"Log-Datei: {log_file}"
                )
                root.destroy()
            except Exception:
                pass  # Wenn tkinter nicht verfügbar, MessageBox überspringen

            # TODO: Hier die tatsächliche Verarbeitung der Datei implementieren
            # Beispiel: Die Datei als Task erstellen und ausführen
            # file_content = file_path.read_text(encoding="utf-8")
            # ... weitere Verarbeitung ...

            # Erfolgreich verarbeitet - Programm beenden
            sys.exit(0)

    # --- Normale CLI-Verarbeitung mit argparse ---
