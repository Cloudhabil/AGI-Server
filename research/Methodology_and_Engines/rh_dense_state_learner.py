...
# GPIA dense-state infrastructure
try:
    ...
    VOXEL_STORAGE_AVAILABLE = True
except ImportError:
    VOXEL_STORAGE_AVAILABLE = False


class RHDenseStateLearner:
    """Learns patterns from Alpha-Professor RH research."""

    def __init__(self, session_dir: Path, config_path: str = "config/dense_state.json"):
        ...
        # Initialize HyperVoxelContract from config
        self.contract = None
        self.storage = None
        if VOXEL_STORAGE_AVAILABLE:
            try:
                config = load_config(config_path)
                self.contract = HyperVoxelContract(
                    shape=tuple(config["voxel"]["shape"]),
                    flatten_order=config["voxel"]["flatten_order"],
                    dtype=config["voxel"]["dtype"]
                )
                self.storage = DenseStateStorage(config=config)
            except Exception as e:
                print(f"[RH] Warning: Could not initialize voxel storage: {e}")

        # Initialize voxel grid with contract's dtype if available
        dtype = self.contract.dtype if self.contract else np.float32
        self.voxel_grid = np.zeros((self.voxel_grid_size, self.voxel_grid_size, self.voxel_grid_size), dtype=dtype)
        self.pattern_history = []
        self.cycle = 0  # Track cycle number for logging
    ...
    def run_learning_cycle(self) -> Dict:
        ...
        # Persist voxel grid to GPIA storage (if available)
        self._persist_voxel_to_storage(voxel_grid, resonance_hash, report)
    ...
    def _persist_voxel_to_storage(self, voxel_grid: np.ndarray, resonance_hash: str, report: Dict) -> None:
        """Persist voxel grid to GPIA dense-state storage (if available)."""
        if not self.contract or not self.storage:
            return

        try:
            # Validate voxel grid
            if not self.contract.validate(voxel_grid):
                print(f"   [RH] Voxel validation failed")
                return

            # Create dense-state log entry
            entry = DenseStateLogEntry(
                vector=self.contract.to_vector(voxel_grid).tolist(),
                mode="voxel",
                shape=list(self.contract.shape),
                dtype=self.contract.dtype,
                flatten_order=self.contract.flatten_order,
                adapter_version="rh_v1",
                adapter_id="rh_pattern_encoder",
                prompt_hash=resonance_hash,
                timestamp=datetime.now(timezone.utc).isoformat(),
                metrics={
                    "resonance_hash": resonance_hash,
                    "success_rate": report.get("success_rate", 0),
                    "cycle": self.cycle,
                    "proposals_analyzed": report.get("proposals_analyzed", 0),
                    "high_promise_count": report.get("high_promise_count", 0)
                }
            )
            ...
            print(f"   [RH] Warning: Could not persist voxel to storage: {e}")

    def get_voxel_history(self, n_recent: int = 5) -> List[np.ndarray]:
        """Retrieve recent voxel grids from storage for pattern analysis."""
        if not self.contract or not self.storage:
            return []

        try:
            entries = self.storage.get_latest(n_recent)
            voxels = []

            for entry in entries:
                if entry.mode == "voxel" and entry.vector:
                    try:
                        voxel = self.contract.unflatten(np.array(entry.vector))
                        voxels.append(voxel)
                    except Exception as e:
                        print(f"   [RH] Could not unflatten voxel: {e}")
                        continue

            return voxels
        except Exception as e:
            print(f"   [RH] Could not retrieve voxel history: {e}")
            return []
