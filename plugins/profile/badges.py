...
REWARDS_FILE = Path(__file__).resolve().parent / "badges.json"
...
def badge_paths(user_id: str) -> List[str]:
    ...
    return [str(ASSETS_DIR / "badges" / f"{bid}.png") for bid in get_rewards(user_id)["badges"]]


def frame_paths(user_id: str) -> List[str]:
    """Return absolute paths to frames for *user_id*."""
    return [str(ASSETS_DIR / "frames" / f"{fid}.png") for fid in get_rewards(user_id)["frames"]]
