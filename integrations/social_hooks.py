...
def instagram_webhook(event: str, data: Dict[str, Any] | None = None) -> None:
    payload = {"event": event, "data": data or {}, "ts": datetime.now(timezone.utc).isoformat()}
...
def notify_error(summary: str) -> None:
    """Post a fatal error summary to Discord and Slack webhooks if configured."""
    payload = {"event": "error", "summary": summary, "ts": datetime.now(timezone.utc).isoformat()}
    _post(os.getenv("DISCORD_WEBHOOK_URL"), payload)
    _post(os.getenv("SLACK_WEBHOOK_URL"), payload)
