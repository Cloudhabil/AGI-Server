...
from agents import (
    ...
    agent_explain_concepts,
    agent_optimize_code,
    agent_resolve_problems,
...
CONFIG_PATH = Path("configs/instructions.json")
...
def dispatch(task: str):
    ...
    if title.startswith("generate ideas"):
        ...
        desc = (
            input("Prototype description (what should it do?): ").strip()
            or "health endpoint + TODO marker"
        )
